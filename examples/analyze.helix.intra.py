...
fl_export = os.path.join(drc_helix, f"{seg}.align.helixparam.eps")
gp = GnuplotPy3.GnuplotPy3()
gp("set encoding utf8")
gp("set terminal postscript eps  size 10, 10 \\")
gp("                             enhanced color \\")
...
for i, _ in enumerate(radius_dict.keys()):
...
for i, (_, v) in enumerate(radius_dict.items()):
    gp(f"{i} {v[seg][atom_type]}")
gp("e")

# Panel 2 - initial phase
gp("set bmargin 1.5")
gp("set ylabel 'Initial phase({\260})'")
## gp("set yrange [70:120]")
for i, _ in enumerate(phase_dict.keys()):
    gp(f"set xtics add (' ' {i}) rotate by 90 right font ',10'")
...
for i, (_, v) in enumerate(phase_dict.items()):
    vv = v[seg][atom_type]
    vv *= 180 / np.pi
    gp(f"{i} {vv}")
gp("e")
...
## gp("set yrange [70:120]")
for i, _ in enumerate(omega_dict.keys()):
    gp(f"set xtics add (' ' {i}) rotate by 90 right font ',10'")
...
for i, (_, v) in enumerate(omega_dict.items()):
    vv = v[seg]
    vv *= 180 / np.pi
    gp(f"{i} {vv}")
gp("e")
...
gp("set tmargin 0.0")
gp("set bmargin 1.5")
gp("set ylabel 'pitch ({\305})'")
## gp("set yrange [0.45:8.5]")
for i, _ in enumerate(pitch_dict.keys()):
    gp(f"set xtics add (' ' {i}) rotate by 90 right font ',10'")
...
for i, (_, v) in enumerate(pitch_dict.items()):
    gp(f"{i} {v[seg]}")
gp("e")
...
gp("set tmargin 0.0")
gp("set bmargin 4.0")
gp("set ylabel 'rmsd ({\305})'")
## gp("set yrange [0.05:0.9]")
for i, lbl in enumerate(rmsd_dict.keys()):
...
for i, (_, v) in enumerate(rmsd_dict.items()):
    gp(f"{i} {v[seg]}")
gp("e")
